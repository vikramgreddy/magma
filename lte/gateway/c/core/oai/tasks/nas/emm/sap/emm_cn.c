...
void handle_apn_mismatch(ue_mm_context_t* const ue_context, int esm_cause) {
  ...
  esm_send_pdn_connectivity_reject(emm_ctx->esm_ctx.esm_proc_data->pti,
...
static int emm_cn_ula_success(emm_cn_ula_success_t* msg_pP) {
  ...
  if (pdn_cid < MAX_APN_PER_UE) {
    ...
    if (!is_pdn_context_exist_for_apn) {
      ...
      if ((mme_app_send_s11_create_session_req(mme_app_desc_p, ue_mm_context,
                                               pdn_cid)) == RETURNok) {
...
//------------------------------------------------------------------------------
static int emm_proc_combined_attach_req(struct emm_context_s* emm_ctx_p,
                                        bstring esm_data) {
  int rc = RETURNerror;
  OAILOG_FUNC_IN(LOG_NAS_EMM);
  ...
  if (emm_ctx_p->attach_type == EMM_ATTACH_TYPE_COMBINED_EPS_IMSI) {
    ...
    if (!(strcmp(non_eps_service_control, "SMS")) ||
      ...
      if (is_mme_ue_context_network_access_mode_packet_only(ue_mm_context_p)) {
        ...
        rc = mme_app_handle_nas_cs_domain_location_update_req(ue_mm_context_p,
                                                              ATTACH_REQUEST);
  ...
  if (attach_proc) {
    ...
    if (rc != RETURNerror) {
      if (IS_EMM_CTXT_PRESENT_OLD_GUTI(emm_ctx) &&
          (memcmp(&emm_ctx->_old_guti, &emm_ctx->_guti,
                  sizeof(emm_ctx->_guti)))) {
        /*
...
static int emm_cn_ula_or_csrsp_fail(const emm_cn_ula_or_csrsp_fail_t* msg) {
  ...
  switch (msg->cause) {
    ...
    case CAUSE_NO_RESOURCES_AVAILABLE:
      ...
      increment_counter("ue_pdn_connectivity_req", 1, 2, "result", "failure",
                        "cause", "no_resources_available");
      break;
    case CAUSE_ALL_DYNAMIC_ADDRESSES_OCCUPIED:
      esm_cause = ESM_CAUSE_INSUFFICIENT_RESOURCES;
      increment_counter("ue_pdn_connectivity_req", 1, 2, "result", "failure",
                        "cause", "all_dynamic_resources_occupied");
      break;
    default:
      esm_cause = ESM_CAUSE_REQUEST_REJECTED_BY_GW;
      break;
...
static int emm_cn_deactivate_dedicated_bearer_req(
  ...
  memcpy(esm_sap.data.eps_bearer_context_deactivate.ebi, msg->ebi,
         sizeof(ebi_t));
...
static int send_attach_accept(struct emm_context_s* emm_ctx_p) {
  ...
  if (rc != RETURNerror) {
    if (IS_EMM_CTXT_PRESENT_OLD_GUTI(emm_ctx_p) &&
        (memcmp(&emm_ctx_p->_old_guti, &emm_ctx_p->_guti,
  ...
  if ((ue_context_p) && (ue_context_p->ecm_state == ECM_CONNECTED)) {
    if (ue_context_p->sgs_context == NULL) {
      OAILOG_WARNING(LOG_NAS_EMM,
                     " Invalid SGS context for IMSI" IMSI_64_FMT "\n", imsi64);
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
    }
