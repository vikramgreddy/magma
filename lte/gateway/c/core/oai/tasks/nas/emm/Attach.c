...
status_code_e emm_proc_attach_request(
  ...
  if (ies->imsi) {
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                "ATTACH REQ (ue_id = " MME_UE_S1AP_ID_FMT
                ") (IMSI = " IMSI_64_FMT ") \n",
                ue_id, imsi64);
  } else if (ies->guti) {
    OAILOG_INFO(LOG_NAS_EMM,
  ...
  OAILOG_INFO(LOG_NAS_EMM, "EMM-PROC:  ATTACH - EPS attach type = %s (%d)\n",
  ...
  if (ies->guti) {  // no need for  && (is_native_guti)
    ...
    // request
    if (guti_ue_mm_ctx) {
      create_new_attach_info(&guti_ue_mm_ctx->emm_context,
                             ue_mm_context->mme_ue_s1ap_id, STOLEN_REF ies,
                             is_mm_ctx_new);
      /*
    ...
    if (imsi_ue_mm_ctx) {
      ...
      if (EMM_REGISTERED == fsm_state) {
        ...
        if (imsi_ue_mm_ctx->emm_context.is_attached) {
          ...
          create_new_attach_info(&imsi_ue_mm_ctx->emm_context,
                                 ue_mm_context->mme_ue_s1ap_id, STOLEN_REF ies,
                                 is_mm_ctx_new);
          // Trigger clean up
  ...
  if (clear_emm_ctxt) {
    ...
    if (ies->voicedomainpreferenceandueusagesetting) {
      memcpy(&new_emm_ctx->volte_params
  ...
  if (ue_mm_context) {
    if (is_nas_specific_procedure_attach_running(&ue_mm_context->emm_context)) {
      ...
      if (emm_ctx->csfbparams.newTmsiAllocated) {
        OAILOG_DEBUG(LOG_NAS_EMM, " CSFB newTmsiAllocated = (%d) true!\n",
...
static int emm_attach_run_procedure(emm_context_t* emm_context) {
  ...
  if (attach_proc) {
    ...
    emm_ctx_set_valid_ue_nw_cap(emm_context,
                                &attach_proc->ies->ue_network_capability);
    if (attach_proc->ies->ms_network_capability) {
      emm_ctx_set_valid_ms_nw_cap(emm_context,
    ...
    if (attach_proc->ies->mob_st_clsMark2) {
      emm_ctx_set_mobile_station_clsMark2(emm_context,
                                          attach_proc->ies->mob_st_clsMark2);
    }
    ...
    if (attach_proc->ies->esm_msg) {
      ...
      if ((rc != RETURNerror) && (esm_sap.err == ESM_SAP_SUCCESS)) {
        ...
        rc = _emm_attach_reject(emm_context,
                                &attach_proc->emm_spec_proc.emm_proc.base_proc);
      } else {
        /*
         * ESM procedure failed and, received message has been discarded or
         * Status message has been returned; ignore ESM procedure failure
...
static void encode_csfb_parameters_attach_accept(
  ...
  if ((emm_ctx->attach_type == EMM_ATTACH_TYPE_COMBINED_EPS_IMSI) &&
    ...
    OAILOG_DEBUG(LOG_NAS_EMM, "Additional update type = (%u)\n",
...
static int emm_send_attach_accept(emm_context_t* emm_context) {
  ...
  if (emm_context) {
    ...
    if (attach_proc) {
      ...
      emm_ctx_set_attribute_valid(emm_context,
      ...
      if (attach_proc->ies->drx_parameter) {
        ...
        emm_ctx_set_valid_drx_parameter(emm_context,
                                        attach_proc->ies->drx_parameter);
      }
      //----------------------------------------
      ...
      if (!IS_EMM_CTXT_PRESENT_GUTI(emm_context)) {
        ...
        if (RETURNok == rc) {
          ...
          memcpy(&emm_sap.u.emm_as.u.establish.tai_list,
                 &emm_context->_tai_list, sizeof(tai_list_t));
        } else {
          OAILOG_ERROR(LOG_NAS_EMM,
    ...
    encode_csfb_parameters_attach_accept(emm_context,
                                         &emm_sap.u.emm_as.u.establish);
    ...
    if (RETURNerror != rc) {
      ...
      nas_start_T3450(attach_proc->ue_id, &attach_proc->T3450,
...
static int emm_attach_accept_retx(emm_context_t* emm_context) {
  ...
  if (attach_proc) {
    ...
    // Encode CSFB parameters
    encode_csfb_parameters_attach_accept_retx(emm_context,
                                              &emm_sap.u.emm_as.u.data);
...
static bool emm_attach_ies_have_changed(mme_ue_s1ap_id_t ue_id,
  ...
  if ((ies1->last_visited_registered_tai) &&
    ...
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " Attach IEs changed: LVR TAI " TAI_FMT "/NULL\n",
  ...
  if ((ies1->ueadditionalsecuritycapability) &&
      (ies2->ueadditionalsecuritycapability)) {
    if (memcmp(ies1->ueadditionalsecuritycapability,
               ies2->ueadditionalsecuritycapability,
...
void proc_new_attach_req(mme_ue_context_t* const mme_ue_context_p,
  ...
  if (attach_info.is_mm_ctx_new) {
    if (ue_context_p->ecm_state == ECM_IDLE) {
      OAILOG_INFO_UE(LOG_NAS_EMM, ue_context_p->emm_context._imsi64,
  ...
  */
  ...
  if (!is_nas_specific_procedure_attach_running(&ue_mm_context->emm_context)) {
    emm_proc_create_procedure_attach_request(ue_mm_context,
                                             STOLEN_REF attach_info.ies);
  }
