...
inline void emm_ctx_set_old_guti(emm_context_t* const ctxt, guti_t* guti) {
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " set old GUTI " GUTI_FMT
               " (present)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id,
...
inline void emm_ctx_set_imei(emm_context_t* const ctxt, imei_t* imei) {
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " set IMEI %s (present)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id,
               imei_str);
}
...
inline void emm_ctx_set_valid_imei(emm_context_t* const ctxt, imei_t* imei) {
  ...
  IMEI_TO_STRING(imei, imei_str, 16);
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " set IMEI %s (valid)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id,
               imei_str);
}
...
inline void emm_ctx_set_imeisv(emm_context_t* const ctxt, imeisv_t* imeisv) {
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " set IMEI_SV (present)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_valid_imeisv(emm_context_t* const ctxt,
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " set IMEI_SV (valid)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_clear_auth_vectors(emm_context_t* const ctxt) {
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " cleared auth vectors \n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
//------------------------------------------------------------------------------
...
inline void emm_ctx_clear_auth_vector(emm_context_t* const ctxt, ksi_t eksi) {
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " cleared auth vector %u \n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id,
               eksi);
...
inline void emm_ctx_clear_security(emm_context_t* const ctxt) {
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT " cleared security context \n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_security_type(emm_context_t* const ctxt,
  ...
  OAILOG_TRACE(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set security context security type %d\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id,
               sc_type);
}
...
inline void emm_ctx_set_security_vector_index(emm_context_t* const ctxt,
  ...
  OAILOG_TRACE(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set security context vector index %d\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id,
               vector_index);
}
...
inline void emm_ctx_set_ue_nw_cap(
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set UE network capability IE (present)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_valid_ue_nw_cap(
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set UE network capability IE (valid)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_ms_nw_cap(
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set MS network capability IE (present)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_valid_ms_nw_cap(
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set MS network capability IE (valid)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_drx_parameter(emm_context_t* const ctxt,
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set current DRX parameter (present)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_valid_drx_parameter(emm_context_t* const ctxt,
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set current DRX parameter (valid)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_eps_bearer_context_status(
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set EPS bearer context status (present)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_set_valid_eps_bearer_context_status(
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " set EPS bearer context status (valid)\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
...
inline void emm_ctx_clear_mobile_station_clsMark2(emm_context_t* const ctxt) {
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
               " cleared mobile station classmark2\n",
               (PARENT_STRUCT(ctxt, struct ue_mm_context_s, emm_context))
                   ->mme_ue_s1ap_id);
}
  ...
  if ((T3450) && (T3450->id == NAS_TIMER_INACTIVE_ID)) {
    ...
    if (NAS_TIMER_INACTIVE_ID != T3450->id) {
      ...
      OAILOG_ERROR(LOG_NAS_EMM,
                   "Could not start T3450 UE " MME_UE_S1AP_ID_FMT " ", ue_id);
    }
  }
}
//------------------------------------------------------------------------------
void nas_start_T3460(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3460) && (T3460->id == NAS_TIMER_INACTIVE_ID)) {
    ...
    if (NAS_TIMER_INACTIVE_ID != T3460->id) {
      ...
      OAILOG_ERROR(LOG_NAS_EMM,
                   "Could not start T3460 UE " MME_UE_S1AP_ID_FMT " ", ue_id);
    }
  }
}
//------------------------------------------------------------------------------
void nas_start_T3470(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3470) && (T3470->id == NAS_TIMER_INACTIVE_ID)) {
    ...
    if (NAS_TIMER_INACTIVE_ID != T3470->id) {
      ...
      OAILOG_ERROR(LOG_NAS_EMM,
                   "Could not start T3470 UE " MME_UE_S1AP_ID_FMT " ", ue_id);
    }
  }
}
//------------------------------------------------------------------------------
void nas_start_T3422(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3422) && (T3422->id == NAS_TIMER_INACTIVE_ID)) {
    ...
    if (NAS_TIMER_INACTIVE_ID != T3422->id) {
      OAILOG_DEBUG(LOG_NAS_EMM, "T3422 started UE " MME_UE_S1AP_ID_FMT "\n",
                   ue_id);
    } else {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "Could not start T3422 UE " MME_UE_S1AP_ID_FMT " ", ue_id);
    }
  }
}
//------------------------------------------------------------------------------
void nas_start_Ts6a_auth_info(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((Ts6a_auth_info) && (Ts6a_auth_info->id == NAS_TIMER_INACTIVE_ID)) {
    ...
    if (NAS_TIMER_INACTIVE_ID != Ts6a_auth_info->id) {
      ...
      OAILOG_ERROR(LOG_NAS_EMM,
                   "Could not start Ts6a_auth_info UE " MME_UE_S1AP_ID_FMT " ",
                   ue_id);
    }
  }
}
