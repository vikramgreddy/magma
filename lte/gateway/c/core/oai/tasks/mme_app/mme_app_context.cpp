...
static bool mme_app_recover_timers_for_ue(const hash_key_t keyP,
...
ue_mm_context_t* mme_ue_context_exists_mme_ue_s1ap_id(
  ...
  hashtable_ts_get(state_imsi_ht, (const hash_key_t)mme_ue_s1ap_id,
                   (void**)&ue_context_p);
  if (ue_context_p) {
    ...
    const guti_t* const guti_p)  //  never NULL, if none put &ue_context_p->guti
{
  ...
  if (INVALID_MME_UE_S1AP_ID != mme_ue_s1ap_id) {
    if (ue_context_p->mme_ue_s1ap_id != mme_ue_s1ap_id) {
      ...
      h_rc = hashtable_ts_remove(mme_state_ue_id_ht,
                                 (const hash_key_t)ue_context_p->mme_ue_s1ap_id,
                                 (void**)&ue_context_p);
      h_rc = hashtable_ts_insert(mme_state_ue_id_ht,
...
//------------------------------------------------------------------------------
void mme_remove_ue_context(mme_ue_context_t* const mme_ue_context_p,
  ...
  directoryd_remove_location_a(ue_context_p->emm_context._imsi64,
   ...
   */
  release_esm_pdn_context(&ue_context_p->emm_context,
                          ue_context_p->mme_ue_s1ap_id);
...
void mme_app_handle_enb_reset_req(
  ...
  if (enb_reset_req->ue_to_reset_list == NULL) {
    OAILOG_ERROR(LOG_MME_APP,
                 "Invalid UE list received in eNB Reset Request\n");
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
  ...
  msg = DEPRECATEDitti_alloc_new_message_fatal(TASK_MME_APP,
...
void mme_ue_context_update_ue_emm_state(mme_ue_s1ap_id_t mme_ue_s1ap_id,
  ...
  if ((ue_context_p->mm_state == UE_UNREGISTERED) &&
    ...
    directoryd_report_location_a(ue_context_p->emm_context._imsi64,
                                 ue_context_p->emm_context._imsi.length);
#endif
    ...
    OAILOG_INFO_UE(LOG_MME_APP, ue_context_p->emm_context._imsi64,
                   "UE STATE - REGISTERED.\n");
  } else if ((ue_context_p->mm_state == UE_REGISTERED) &&
...
//------------------------------------------------------------------------------
{
  ...
  if (ue_mm_context->ecm_state == ECM_IDLE) {
    ...
    if (ue_mm_context->ue_context_rel_cause == S1AP_SCTP_SHUTDOWN_OR_RESET) {
      ...
      mme_app_itti_ue_context_release(ue_mm_context,
                                      ue_mm_context->ue_context_rel_cause);
      OAILOG_WARNING_UE(
  ...
  for (int bid = 0; bid < BEARERS_PER_UE; bid++) {
    if (ue_mm_context->bearer_contexts[bid]) {
      esm_ebr_stop_timer(&ue_mm_context->emm_context,
    ...
    if (bearer_context_p) {
      ...
      if (ue_context_p->pdn_contexts[pdn_cid] &&
        ...
        if (bearer_context_p->esm_ebr_context.status ==
          ...
          default_eps_bearer_activate_t3485_handler(NULL, NAS_TIMER_INACTIVE_ID,
                                                    (void*)&timer_args);
        } else {  // Invoke callback registered for default bearer's
                  // deactivation procedure
          if (bearer_context_p->esm_ebr_context.status ==
              ESM_EBR_INACTIVE_PENDING) {
            eps_bearer_deactivate_t3495_handler(NULL, NAS_TIMER_INACTIVE_ID,
                                                (void*)&timer_args);
          }
        }
      } else {
...
static void mme_app_handle_timer_for_unregistered_ue(
  ...
  if ((attach_proc) && (is_nas_attach_accept_sent(attach_proc))) {
    OAILOG_INFO_UE(LOG_MME_APP, ue_context_p->emm_context._imsi64,
                   ...
                   ue_context_p->mme_ue_s1ap_id);
    emm_proc_nw_initiated_detach_request(ue_context_p->mme_ue_s1ap_id,
                                         NW_DETACH_TYPE_RE_ATTACH_REQUIRED);
  } else {
    OAILOG_INFO_UE(LOG_MME_APP, ue_context_p->emm_context._imsi64,
...
void mme_app_remove_stale_ue_context(
  ...
  OAILOG_FUNC_IN(LOG_MME_APP);
  enb_s1ap_id_key_t enb_s1ap_id_key = INVALID_ENB_UE_S1AP_ID_KEY;
  MME_APP_ENB_S1AP_ID_KEY(enb_s1ap_id_key, s1ap_remove_stale_ue_context->enb_id,
                          s1ap_remove_stale_ue_context->enb_ue_s1ap_id);
  if (enb_s1ap_id_key == INVALID_ENB_UE_S1AP_ID_KEY) {
    OAILOG_ERROR(LOG_MME_APP, "Received invalid enb_s1ap_id_key :%lx\n",
                 enb_s1ap_id_key);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
